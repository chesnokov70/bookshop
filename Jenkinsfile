    ...
    stage('Copy Files to EC2') {
      steps {
        sshagent(credentials: ['ssh_instance_key']) {
         ...
         scp /var/lib/jenkins/workspace/My_Lessons_Folder/bookshop/docker-compose.tmpl ${EC2_USER}@${HOST}:/opt
         scp /var/lib/jenkins/workspace/My_Lessons_Folder/bookshop/promtail-config.yaml ${EC2_USER}@${HOST}:/opt
         """
        }
      }
    }    
